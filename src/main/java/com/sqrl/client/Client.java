...
public class Client {
	...
	public static void main(String[] args) throws Base64DecodingException, GeneralSecurityException, PasswordVerifyException {
		// stored user profile information, This should be loaded off disk for an existing 
	...
	public static SQRLAuthentication createAuthentication(String siteURL, SQRLIdentity identity) throws SQRLException {
		...
		// This is the expensive operation and its parameters should be tuned so that this 
		...
		// STEP 2: Check the sha256 hash of the result from STEP 1 verse the stored passwordVerify
		// value.
		...
		// STEP 3: XOR the master identity key from the SQRLIdentity with the result from STEP 1
	...
	private static byte[] scrypt(String password, SQRLIdentity sqrlIdentity) {
		// CPU Cost - This should be tuned so that this takes between 1 and 2 seconds to calculate
